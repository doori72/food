...
public class DeliveryListViewHandler {
    ...
    public void whenCookFinished_then_CREATE_1 (@Payload CookFinished cookFinished) {
        try {
            ...
            deliveryList.setOrderId(cookFinished.getOrderId());
            deliveryList.setAddress(cookFinished.getAddress());
            deliveryList.setCustomerId(cookFinished.getCustomerId());
            deliveryList.setFoodId(cookFinished.getFoodId());
            // view 레파지 토리에 save
